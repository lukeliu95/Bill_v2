...
企业数据存放在 `data/enterprises.db`（SQLite + FTS5），由 gBizINFO CSV（579万行）导入。
- 结构化索引：都道府県、従業員数、資本金、設立日 — 查询毫秒级
- FTS5 全文索引：商号、事業概要、事業種目 — 关键词搜索毫秒级
- 检索脚本：`scripts/enterprise_search.py`（三层漏斗）
- 导入脚本：`scripts/import_csv_to_sqlite.py`（DB 不存在时运行，约 40 秒）
- **注意**：事業概要填写率仅 1.5%，従業員数仅 2.4%，新兴 SaaS 企业覆盖不足
